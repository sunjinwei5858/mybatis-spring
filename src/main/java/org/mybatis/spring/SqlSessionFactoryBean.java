...
/**
 * 通过英文注释可以得出：
 * mybatis通过FactoryBean来注入SqlSessionFactory对象，这个SqlSessionFactory是线程安全的，可以应用在spring容器的上下文。
 * FactoryBean：spring提供这个FactoryBean接口，一般用于和第三方框架整合，进行注入对象，这个接口有三个方法
 * getObject();
 * getObjectType();
 * isSingleton()
 * SqlSessionFactoryBean 类作为我们分析源码的入口，该类实现了 InitializingBean 接口，
...
public class SqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>, InitializingBean, ApplicationListener<ApplicationEvent> {
    ...
    /**
     * SqlSessionFactoryBean 类作为我们分析源码的入口，该类实现了 InitializingBean 接口，在 Bean 初始化的时候，
     * 会执行 afterPropertiesSet() 方法，最后会调用 buildSqlSessionFactory() 方法创建 SqlSessionFactory。
     * {@inheritDoc}
     */
    @Override
